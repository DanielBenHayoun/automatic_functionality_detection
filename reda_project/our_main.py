...
def analyze_func(proj, fun, cfg):
    ...
    })#call_state is the init state of $fun
    ...
    sm = proj.factory.simulation_manager(call_state)#creating simulation_manager for $call_state
    sm.use_technique(angr.exploration_techniques.LoopSeer(cfg=cfg, bound=2))#make the simulation iterate 2 times max per loop
    global start_time
    start_time = time.time()
    sm.run(until=time_limit_check)#run the simulation for 1 hour
...
def block_to_ins(block: angr.block.Block): #convert block to instructions in the form of [|,mov|reg1|reg2|    ,add|reg1|reg2|	]	for example
    result = []
    for ins in block.capstone.insns: #iterate over $block instructions after disassemble 
        op_str = ins.op_str
        operands = op_str.strip(" ").split(",")
        operands = [i.strip().replace("[","").replace("]", "") for i in operands if i != ""] #operands is the operands of $ins without [,] and spaces
        parsed_ins = [ins.mnemonic] + list(filter(None, operands))
        result.append("|".join(parsed_ins).replace(" ", "|") + "|\t")#add to $result for example:"move reg1,reg2" --> "mov|reg1|reg2|	"
...
def generate_dataset(train_binaries, dataset_name):
    ...
    os.makedirs(dataset_dir, exist_ok=True) #make new directory with the name datasets/{dataset_name}
...
def analyse_binary(analysed_funcs, binary_name, dataset_dir):
    ...
    proj = angr.Project(binary_name, auto_load_libs=False)#load binary project
    cfg = proj.analyses.CFGFast() #cfg is the control flow graph of $binary_name
    binary_name = os.path.basename(binary_name)
    binary_dir = os.path.join(dataset_dir, f"{binary_name}")
    os.makedirs(binary_dir, exist_ok=True) #make for each binary a folder in $dataset_dir
    funcs = get_cfg_funcs(proj, binary_name, excluded) #filter only relevant fucntions
    print(f"{binary_name} have {len(funcs)} funcs")
    for test_func in funcs:  #iterate on all the relevant functions in $binary_name
        if test_func.name in analysed_funcs:
            print(f"skipping {test_func.name}")
            continue
        print(f"analyzing {binary_name}/{test_func.name}")
        output = open(os.path.join(binary_dir, f"{test_func.name}"), "w") #make new file for func
        # bases_dict.clear()
        # replacement_dict.clear()
        analysed_funcs.add(test_func.name) #add $test_func to $analysed_funcs
        try:
...
def sm_to_output(sm: angr.sim_manager.SimulationManager, output_file, func_name):
    counters = {'mem': itertools.count(), 'ret': itertools.count()} #init $counters
    var_map = {} #init $var_map
    skipped_lines = 0
    constants_mapper = dict() #init $constants_mapper
    constants_counter = itertools.count()
    proj = sm._project
    for exec_paths in sm.stashes.values():
        for exec_path in exec_paths:
            blocks = [proj.factory.block(baddr) for baddr in exec_path.history.bbl_addrs] #list of basic blocks executed by the state in the past
            processsed_code = "|".join(list(filter(None, map(block_to_ins, blocks))))#create a list of the form["|",[|,mov|reg1|reg2|    ,add|reg1|reg2|	],...]
...
def main(): #run generate_dataset for the i'th binary in "coreutils_bins"
    parser = argparse.ArgumentParser()
    # we did this in order to parallelize the analysis process    
    parser.add_argument("--binary_idx", type=int, required=True)#to run main we need to give argument --binary_idx=a where a is int
    args = parser.parse_args()
    binaries = os.listdir("coreutils_bins")#binaries=all the binaries files in "coreutils_bins"
    binaries.sort()
    binaries = [f"coreutils_bins/{binary}" for binary in binaries]#binaries=all the paths of the binaries files in "coreutils_bins"
