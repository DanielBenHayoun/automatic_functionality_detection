
undefined * mbscasecmp(byte *param_1,byte *param_2)

{
  byte *pbVar1;
  byte bVar2;
  byte bVar3;
  int iVar4;
  int iVar5;
  ulong uVar6;
  long *plVar7;
  undefined8 uVar8;
  long lVar9;
  undefined *puVar10;
  long lVar11;
  byte *pbVar12;
  byte *pbVar13;
  byte *pbVar14;
  char *pcVar15;
  long in_FS_OFFSET;
  bool bVar16;
  bool bVar17;
  bool bVar18;
  byte bVar19;
  undefined8 uStack196;
  char cStack188;
  byte *pbStack184;
  ulong uStack176;
  char cStack168;
  uint auStack164 [7];
  char cStack136;
  undefined8 uStack132;
  char cStack124;
  byte *pbStack120;
  ulong uStack112;
  char cStack104;
  int aiStack100 [9];
  long lStack64;
  
  bVar19 = 0;
  lStack64 = *(long *)(in_FS_OFFSET + 0x28);
  if (param_1 == param_2) {
    uVar6 = 0;
  }
  else {
    uVar6 = func_0x00102520();
    if (1 < uVar6) {
      bVar16 = false;
      uStack196 = 0;
      cStack136 = '\0';
      uStack132 = 0;
      pbStack184 = param_1;
      pbStack120 = param_2;
code_r0x0010a0f9:
      cStack124 = '\0';
      cStack188 = '\0';
      if (!bVar16) {
        if ((*(uint *)(is_basic_table + (ulong)(*pbStack184 >> 5) * 4) >> ((uint)*pbStack184 & 0x1f)
            & 1) == 0) {
          iVar5 = func_0x00102990(&uStack196);
          if (iVar5 != 0) {
            bVar16 = true;
            goto code_r0x0010a1a4;
          }
          goto code_r0x0010a600;
        }
        uStack176 = 1;
        auStack164[0] = SEXT14((char)*pbStack184);
        cStack168 = '\x01';
code_r0x0010a137:
        cStack188 = '\x01';
        if (auStack164[0] != 0) goto code_r0x0010a145;
code_r0x0010a445:
        if (cStack124 == '\0') {
          if (cStack136 == '\0') {
            if ((*(uint *)(is_basic_table + (ulong)(*pbStack120 >> 5) * 4) >>
                 ((uint)*pbStack120 & 0x1f) & 1) != 0) {
              uStack112 = 1;
              aiStack100[0] = (int)(char)*pbStack120;
              goto code_r0x0010a493;
            }
            iVar5 = func_0x00102990(&uStack132);
            if (iVar5 == 0) goto code_r0x0010a600;
            cStack136 = '\x01';
          }
          uVar8 = func_0x00102520();
          pbVar13 = pbStack120;
          uVar8 = strnlen1(pbStack120,uVar8);
          uStack112 = rpl_mbrtowc(aiStack100,pbVar13,uVar8);
          if (uStack112 + 2 < 2) goto code_r0x0010a3d8;
          if (uStack112 == 0) {
            uStack112 = 1;
            if (*pbStack120 != 0) goto code_r0x0010a624;
            if (aiStack100[0] != 0) goto code_r0x0010a5fb;
          }
        }
        else {
          if (cStack104 == '\0') goto code_r0x0010a3d8;
        }
code_r0x0010a493:
        uVar6 = (ulong)-(uint)(aiStack100[0] != 0);
        goto code_r0x00109fe8;
      }
code_r0x0010a1a4:
      uVar8 = func_0x00102520();
      pbVar13 = pbStack184;
      uVar8 = strnlen1(pbStack184,uVar8);
      uStack176 = rpl_mbrtowc(auStack164,pbVar13,uVar8,&uStack196);
      if (uStack176 != 0xffffffffffffffff) {
        if (uStack176 == 0xfffffffffffffffe) {
          uStack176 = func_0x00102530(pbStack184);
          cStack168 = '\0';
          goto code_r0x0010a145;
        }
        if (uStack176 != 0) {
code_r0x0010a211:
          cStack168 = '\x01';
          iVar5 = func_0x00102990(&uStack196);
          if (iVar5 != 0) {
            bVar16 = false;
          }
          goto code_r0x0010a137;
        }
        uStack176 = 1;
        if (*pbStack184 != 0) goto code_r0x0010a624;
        if (auStack164[0] == 0) goto code_r0x0010a211;
code_r0x0010a5fb:
        mbuiter_multi_next_part_0();
code_r0x0010a600:
        func_0x00102600(&UNK_001214a4,&UNK_00121496,0x96,__PRETTY_FUNCTION___5133);
        goto code_r0x0010a61f;
      }
      uStack176 = 1;
      cStack168 = '\0';
code_r0x0010a145:
      cStack188 = '\x01';
      if (cStack124 == '\0') {
        if (cStack136 == '\0') {
          if ((*(uint *)(is_basic_table + (ulong)(*pbStack120 >> 5) * 4) >>
               ((uint)*pbStack120 & 0x1f) & 1) != 0) {
            uStack112 = 1;
            aiStack100[0] = (int)(char)*pbStack120;
            cStack104 = '\x01';
            cStack124 = '\x01';
            goto code_r0x0010a0a3;
          }
          iVar5 = func_0x00102990(&uStack132);
          if (iVar5 == 0) goto code_r0x0010a600;
          cStack136 = '\x01';
        }
        uVar8 = func_0x00102520();
        pbVar13 = pbStack120;
        uVar8 = strnlen1(pbStack120,uVar8);
        uStack112 = rpl_mbrtowc(aiStack100,pbVar13,uVar8,&uStack132);
        if (uStack112 == 0xffffffffffffffff) {
          uStack112 = 1;
          cStack104 = '\0';
          cStack124 = '\x01';
        }
        else {
          if (uStack112 != 0xfffffffffffffffe) {
            if (uStack112 == 0) {
              uStack112 = 1;
              if (*pbStack120 != 0) goto code_r0x0010a624;
              if (aiStack100[0] != 0) goto code_r0x0010a5fb;
            }
            cStack104 = '\x01';
            iVar5 = func_0x00102990(&uStack132);
            if (iVar5 == 0) {
              cStack124 = '\x01';
            }
            else {
              cStack136 = '\0';
              cStack124 = '\x01';
            }
            goto code_r0x0010a0a3;
          }
          uStack112 = func_0x00102530(pbStack120);
          cStack104 = '\0';
          cStack124 = '\x01';
        }
code_r0x0010a15b:
        if (cStack168 != '\0') goto code_r0x0010a3d8;
        if (uStack176 != uStack112) {
          if (uStack176 < uStack112) {
            iVar5 = func_0x00102680();
            if (iVar5 < 1) {
code_r0x0010a3d8:
              uVar6 = 0xffffffff;
              goto code_r0x00109fe8;
            }
          }
          else {
            iVar5 = func_0x00102680(pbStack184,pbStack120,uStack112);
            if (iVar5 < 0) goto code_r0x0010a3d8;
          }
          goto code_r0x0010a3c0;
        }
        uVar6 = func_0x00102680();
      }
      else {
        if (cStack104 == '\0') goto code_r0x0010a15b;
code_r0x0010a0a3:
        if (aiStack100[0] == 0) {
          if (cStack188 == '\0') {
            if (!bVar16) {
              if ((*(uint *)(is_basic_table + (ulong)(*pbStack184 >> 5) * 4) >>
                   ((uint)*pbStack184 & 0x1f) & 1) != 0) {
                uStack176 = 1;
                auStack164[0] = SEXT14((char)*pbStack184);
                cStack168 = '\x01';
                cStack188 = '\x01';
                goto code_r0x0010a439;
              }
              iVar5 = func_0x00102990(&uStack196);
              if (iVar5 == 0) goto code_r0x0010a600;
            }
            uVar8 = func_0x00102520();
            pbVar13 = pbStack184;
            uVar8 = strnlen1(pbStack184,uVar8);
            uStack176 = rpl_mbrtowc(auStack164,pbVar13,uVar8,&uStack196);
            if ((uStack176 != 0xffffffffffffffff) && (uStack176 != 0xfffffffffffffffe)) {
              if (uStack176 == 0) {
                uStack176 = 1;
                if (*pbStack184 != 0) goto code_r0x0010a624;
                if (auStack164[0] != 0) goto code_r0x0010a5fb;
              }
              cStack168 = '\x01';
              iVar5 = func_0x00102990(&uStack196);
              if (iVar5 == 0) {
                cStack188 = '\x01';
              }
              else {
                cStack188 = '\x01';
              }
code_r0x0010a439:
              if (auStack164[0] == 0) goto code_r0x0010a445;
            }
          }
          else {
            if (cStack168 != '\0') goto code_r0x0010a439;
          }
code_r0x0010a3c0:
          uVar6 = 1;
          goto code_r0x00109fe8;
        }
        if (cStack168 == '\0') goto code_r0x0010a3c0;
        iVar5 = func_0x001028e0((ulong)auStack164[0]);
        iVar4 = func_0x001028e0();
        uVar6 = (ulong)(uint)(iVar5 - iVar4);
      }
      if ((int)uVar6 != 0) goto code_r0x00109fe8;
      pbStack184 = pbStack184 + uStack176;
      pbStack120 = pbStack120 + uStack112;
      goto code_r0x0010a0f9;
    }
    plVar7 = (long *)func_0x001029f0();
    lVar9 = *plVar7;
    do {
      bVar2 = *param_1;
      if ((*(byte *)(lVar9 + 1 + (ulong)bVar2 * 2) & 1) != 0) {
        plVar7 = (long *)func_0x001029e0();
        bVar2 = *(byte *)(*plVar7 + (ulong)bVar2 * 4);
      }
      bVar3 = *param_2;
      if ((*(byte *)(lVar9 + 1 + (ulong)bVar3 * 2) & 1) != 0) {
        plVar7 = (long *)func_0x001029e0();
        bVar3 = *(byte *)(*plVar7 + (ulong)bVar3 * 4);
      }
      if (bVar2 == 0) {
        uVar6 = (ulong)-(uint)bVar3;
        goto code_r0x00109fe8;
      }
      param_1 = param_1 + 1;
      param_2 = param_2 + 1;
    } while (bVar2 == bVar3);
    uVar6 = (ulong)((uint)bVar2 - (uint)bVar3);
  }
code_r0x00109fe8:
  if (lStack64 == *(long *)(in_FS_OFFSET + 0x28)) {
    return (undefined *)uVar6;
  }
code_r0x0010a61f:
  func_0x00102560();
code_r0x0010a624:
  pbVar13 = &UNK_001214bb;
  func_0x00102600(&UNK_001214bb,&UNK_00121496,0xb2,__PRETTY_FUNCTION___5133);
  if (pbVar13 == (byte *)0x0) {
    pcVar15 = "A NULL argv[0] was passed through an exec system call.\n";
    iVar5 = 1;
    func_0x00102960(&UNK_00121a58,1,0x37,stderr);
    func_0x00102400();
    pbVar13 = (byte *)locale_charset();
    if ((*pbVar13 & 0xdf) == 0x55) {
      if (((((pbVar13[1] & 0xdf) == 0x54) && ((pbVar13[2] & 0xdf) == 0x46)) && (pbVar13[3] == 0x2d))
         && ((pbVar13[4] == 0x38 && (pbVar13[5] == 0)))) {
        puVar10 = &UNK_00121a9c;
        if (*pcVar15 != '`') {
          puVar10 = &UNK_00121aa0;
        }
        return puVar10;
      }
    }
    else {
      if ((((*pbVar13 & 0xdf) == 0x47) && ((pbVar13[1] & 0xdf) == 0x42)) &&
         ((pbVar13[2] == 0x31 &&
          ((((pbVar13[3] == 0x38 && (pbVar13[4] == 0x30)) && (pbVar13[5] == 0x33)) &&
           ((pbVar13[6] == 0x30 && (pbVar13[7] == 0)))))))) {
        puVar10 = &UNK_00121aa4;
        if (*pcVar15 != '`') {
          puVar10 = &UNK_00121aa8;
        }
        return puVar10;
      }
    }
    puVar10 = &UNK_00121aab;
    if (iVar5 != 9) {
      puVar10 = &UNK_001214d0;
    }
    return puVar10;
  }
  lVar9 = func_0x001025d0();
  if (lVar9 != 0) {
    pbVar1 = (byte *)(lVar9 + 1);
    pbVar12 = pbVar1 + -(long)pbVar13;
    bVar16 = pbVar12 < (byte *)0x6;
    bVar18 = pbVar12 == (byte *)0x6;
    if (6 < (long)pbVar12) {
      lVar11 = 7;
      pbVar12 = (byte *)(lVar9 + -6);
      pbVar14 = &UNK_00121a90;
      do {
        if (lVar11 == 0) break;
        lVar11 = lVar11 + -1;
        bVar16 = *pbVar12 < *pbVar14;
        bVar18 = *pbVar12 == *pbVar14;
        pbVar12 = pbVar12 + (ulong)bVar19 * -2 + 1;
        pbVar14 = pbVar14 + (ulong)bVar19 * -2 + 1;
      } while (bVar18);
      bVar17 = false;
      bVar16 = (!bVar16 && !bVar18) == bVar16;
      if (bVar16) {
        lVar11 = 3;
        pbVar13 = pbVar1;
        pbVar12 = &UNK_00121a98;
        do {
          if (lVar11 == 0) break;
          lVar11 = lVar11 + -1;
          bVar17 = *pbVar13 < *pbVar12;
          bVar16 = *pbVar13 == *pbVar12;
          pbVar13 = pbVar13 + (ulong)bVar19 * -2 + 1;
          pbVar12 = pbVar12 + (ulong)bVar19 * -2 + 1;
        } while (bVar16);
        pbVar13 = pbVar1;
        if ((!bVar17 && !bVar16) == bVar17) {
          pbVar13 = (byte *)(lVar9 + 4);
          program_invocation_short_name = pbVar13;
        }
      }
    }
  }
  program_invocation_name = pbVar13;
  program_name = pbVar13;
  return (undefined *)lVar9;
}

